...
## Read in data.
@load "$(@__DIR__)/../steady_state_double_channel.jld" u_steady params xx yy T0  
...
P.-=minimum(P) # get rid of strange negative values

p3 = contour(xx,yy,P,levels=range(0.01,stop=maximum(P),length=nlevels),linewidths=0.8)
# Make an invisible colorbar, so that everything lines up.
figure(2)
cbar_ax = contourf(P) # want colour bar. Copy T example - MJ
figure(1)
colorbar(cbar_ax,ticks=round.(range(0.0,stop=maximum(P),length=4),digits=2))
#cbar_ax = colorbar()
...
# Jy = rotl90(Jy)
ψ = flowfun(Jx,Jy)
contour(xx,yy,ψ,range(minimum(ψ),stop=maximum(ψ),length=8),colors="g",linewidths=1.0)
# Make an invisible colorbar, so that everything lines up.
cbar_ax = colorbar()
cbar_ax.ax.set_visible(false)
quiver(xx[1:stride:end-1],yy[1:stride:end-1],Jx[1:stride:end-1,1:stride:end-1],
    Jy[1:stride:end-1,1:stride:end-1],scale=4,width=0.005,headwidth=4,zorder=2)
xticks([])
ylabel(L"y/l")
...
quiver(xx[1:stride:end],yy[1:stride:end],-nablaT_x[1:stride:end,1:stride:end],
    -nablaT_y[1:stride:end,1:stride:end],zorder=2,width=0.005,headwidth=5,scale=3.0)
ylabel(L"y/l")
xlabel(L"x/L")
## Save the plot into the figures directory.
savefig("$(@__DIR__)/../figures/double_channel_offset.pdf",bbox_inches="tight")
